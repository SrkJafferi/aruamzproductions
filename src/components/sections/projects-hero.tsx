...
export function ProjectsHero() {
  ...
  useEffect(() => {
    ...
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      el.querySelectorAll<HTMLElement>("[data-prj-hero]").forEach((node) => {
    ...
    const ctx = gsap.context(() => {
      ...
      /* Type leaves; the scrim only fades. A full-bleed scrim that travels drags
         its own bottom edge into frame and uncovers the join into the gallery —
         the same defect already fixed on the home, About and Services heroes. */
      gsap.to("[data-prj-hero-exit]", {
        opacity: 0,
        y: -34,
        ease: "none",
        scrollTrigger: { trigger: el, start: "45% top", end: "bottom top", scrub: true },
      });
      gsap.to("[data-prj-hero-scrim-fade]", {
        opacity: 0,
        ease: "none",
        scrollTrigger: { trigger: el, start: "45% top", end: "bottom top", scrub: true },
      });
      ...
      stopIntro = onAppReady(() =>
        ctx.add(() => {
          ...
          tl.from(frame.current, { scale: 1.14, duration: 2, ease: "power3.out" })
            .from("[data-prj-title]", { yPercent: 116, duration: 1.4, ease: "expo.out" }, 0.2)
            .from("[data-prj-crumb]", { y: 14, opacity: 0, duration: 0.9 }, 0.7)
            .from("[data-prj-rule]", { scaleX: 0, duration: 1.4 }, 0.75)
            .from("[data-prj-mark]", { scale: 0.6, opacity: 0, duration: 1 }, 0.85)
            .from("[data-prj-cta]", { y: 18, opacity: 0, duration: 0.9 }, 1.05);
        }),
      );
    }, el);
  ...
  return (
    /* Centred and breadcrumbed, matching About and Our Services — the interior
       pages have to open the same way or the site reads as three sites. */
    <section
      ref={root}
    ...
    >
      <div aria-hidden className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          />
          <div data-prj-hero-scrim-fade className="absolute inset-0">
            <div
              className="absolute inset-0"
              style={{
                background:
                  "radial-gradient(82% 66% at 50% 48%, rgb(8 8 8 / 0.72) 0%, rgb(8 8 8 / 0.58) 45%, rgb(8 8 8 / 0.20) 78%, transparent 100%)",
              }}
            />
            <div
              className="absolute inset-0"
            ...
            />
            {/* Flat ellipse across the title row. The plate is a lit warehouse
                set, so the brightest part of the frame lands where the gold H1
                does; the correction is local rather than a deeper global wash,
                which would flatten the whole photograph into a silhouette. */}
            <div
              className="absolute inset-0"
              style={{
                background:
      ...
      >
        ...
        >
          <span data-prj-hero data-prj-title className="block">
            {projectsHero.heading}
          </span>
        </h1>

        <nav data-prj-hero data-prj-crumb aria-label="Breadcrumb" className="mt-6">
          <ol className="flex flex-wrap items-center justify-center gap-2 font-mono text-[0.6875rem] font-semibold tracking-[0.24em] text-white/60 uppercase">
            {projectsHero.breadcrumb.map((crumb, index) => {
              const last = index === projectsHero.breadcrumb.length - 1;
              return (
